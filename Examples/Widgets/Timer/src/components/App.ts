...
export default class TimerComponent extends LitElement {
  ...
  @property({ type: String, attribute: "minutes-color" }) minutesColor =
    "#73A321";
  @property({ type: String, attribute: "seconds-color" }) secondsColor =
    "#875AE0";
  ...
  get storedStartTime() {
    return (
      window.localStorage.getItem("timer-start") || DateTime.local().toString()
    );
  }
  get storedDuration() {
  ...
  connectedCallback() {
    ...
    setInterval(() => {
      if (!this.paused) {
        ...
        let [hours, minutes, seconds] = this.durationData
          .toFormat("hh:mm:ss")
          .split(":");
  ...
  setDuration = (startTime: string, duration: string) => {
    ...
    this.durationData = Duration.fromObject(
      remaining.toDuration(["hours", "minutes", "seconds"]).toObject()
    );
  };
    ...
    return html`
      <md-tooltip message="Timer expires at ${this.endTime}">
        <div
          class="container"
          role="timer"
          aria-label="${this.remainingHours} hours, ${this
            .remainingMinutes} minutes, ${this
            .remainingSeconds} seconds remaining"
        >
          <time-circle
            time-limit="8"
            time-left=${Number(this.remainingHours)}
            color=${this.hoursColor}
          >
            <div
              slot="time-unit"
              class="hours"
              aria-label="${this.remainingHours} hours"
            >
          ...
          <time-circle
            time-limit="60"
            time-left=${Number(this.remainingMinutes)}
            color=${this.minutesColor}
          >
            <div
              slot="time-unit"
              class="minutes"
              aria-label="${this.remainingMinutes} minutes"
            >
              ${this.remainingMinutes}m
            </div>
          </time-circle>
          <time-circle
            time-limit="60"
            time-left=${Number(this.remainingSeconds)}
            color=${this.secondsColor}
          >
            <div
              slot="time-unit"
              class="seconds"
              aria-label="${this.remainingSeconds} seconds"
            >
